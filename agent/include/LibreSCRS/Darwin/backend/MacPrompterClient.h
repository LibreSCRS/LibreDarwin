...
public:
    ...
    // retry counter). Empty (the default) is makeDefaultPrompterVerifier()
    // (PeerPolicy.h): the prompter's signing id + App-Group entitlement, the
    // shared PeerCodeSigning gate, symmetric to the prompter authenticating
