...
void SignBatchOperation::doWork()
{
    ...
    // before touching any torn-down member) — same discipline as
    // SignOperation.
    ...
    if (result.outcome == BatchSignFlow::Outcome::InvalidRequest) {
        // Defense-in-depth only: the dispatch validates the document count at
        // method entry (mirroring BatchSignFlow's own
    ...
    // BEFORE Finished, mirroring every sibling typed result. Unlike Sign,
    ...
    // per-row codes are exactly what a client needs to explain the failure
