...
public:
    struct Deps
    {
        ...
        // resolved ONCE at the dispatch entry exactly like a single Sign's own
        // params — `inputDocument`/`displayName` are ignored here (each entry
        // of `documents` below supplies its own), mirroring
        // BatchSignFlowDeps::params.
        SignParams params;
...
private:
    ...
    // — one per dispatch (see backend/SocketFrontend.cpp's handleSignBatch) —
    // so it is NEVER shared or reused across batches; BatchSignFlow's own
    // scope guard wipes it on every run() exit (success, halt, cancel,
    // invalid-request, or an unwinding exception).
